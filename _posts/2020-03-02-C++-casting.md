...
예를 들어 다름과 같이 클래스 계층이 구성되어 있는꼉우를 살펴본다.
...
반면 레퍼런스의 경우, 다음과 같이 적용하면 익셉션이 발생한다.
...
차이점으로는 dynamic_cast()는 실행 시간ㄴ에 타입 검ㅁ사를 수행한는 반면, static_cast()나 reinterpret_cast()의 경우는 문제가 되는 타입도 그냥 캐스팅 해버린다는 것이다.
...
C++ 타입규칙에서 허용핮지 않더라도 상황에 따라 캐스팅하는 것이 적합할 때 쩍요할 수 있다.
